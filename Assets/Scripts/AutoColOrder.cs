...
public class AutoColOrder : MonoBehaviour
{
    ...
    private static Dictionary<Transform, Vector2> orderedPositions = new Dictionary<Transform, Vector2>();
    ...
    void CalculateOrderedPositions()
    {
        ...
        FindLinkedQuads("Y");
        ReorderColumns(lastResult.Item2, "Y");
    ...
    {
        if (SPM != null && columnsParent != null)
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                ...
                if (parentName == "X")
                {
                    currentArray[i].AddRange(FindAllQuadsInChild(columnName, 1));
                }
                else if (parentName == "Y")
                {
                    currentArray[i].AddRange(FindAllQuadsInChild(columnName, 0));
                }
    ...
    private void ReorderColumns(List<int> result_order, string axisName)
    {
        ...
        // 初期位置を保存
        Dictionary<Transform, Vector2> initialPositions = new Dictionary<Transform, Vector2>();
