...
def compute_metrics(dataset, var_indices):
    ...
    for simplex in tri.simplices:
        for i in range(2):
            edge = tuple(sorted([simplex[i], simplex[(i+1)%2]]))
            edges.add(edge)
    ...
    alpha_area = hull.area
    alpha_perimeter = np.sum(hull.area)
    c = (4 * np.pi) ** (1/2)
    skinny = 1 - c * np.sqrt(alpha_area) / alpha_perimeter
...
def initialize_data(json_data):
    ...
    results = []
    for var_indices in combinations(range(data.shape[1]), 2):
